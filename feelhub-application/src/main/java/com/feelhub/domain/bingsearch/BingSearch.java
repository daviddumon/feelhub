...
public class BingSearch {
    ...
    private void doBingSearch(final BingRequest bingRequest) {
        ...
        final List<WebTopic> images = getImages(topic, query);
        setIllustrationForTopic(topic, images);
        bindImagesToQuery(images, query);
        bingRelationBinder.bind(topic, images);
    }

    private List<WebTopic> getImages(final Topic topic, final String query) {
        ...
        for (final String illustration : illustrations) {
            final WebTopic image = createImage(illustration);
            images.add(image);
        }
        return images;
    }

    private WebTopic createImage(final String illustration) {
        final WebTopic image = topicFactory.createWebTopic(illustration, WebTopicType.Image);
    ...
    private void setIllustrationForTopic(final Topic topic, final List<WebTopic> images) {
        if (topic.getIllustrationLink().isEmpty() && !images.isEmpty()) {
            topic.setIllustrationLink(images.get(0).getIllustrationLink());
        }
    }

    private void bindImagesToQuery(final List<WebTopic> images, final String query) {
