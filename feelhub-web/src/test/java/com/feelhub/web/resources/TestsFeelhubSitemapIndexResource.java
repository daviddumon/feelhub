...
public class TestsFeelhubSitemapIndexResource {
    ...
    @Test
    public void feelhubSitemapIndexResourceIsMapped() {
    ...
    public void doesUseSitemapLink() throws IOException {
        when(feelhubSitemapModuleLink.get("/sitemap_index_00001.xml")).thenReturn(IOUtils.toInputStream("sitemap"));
        final ClientResource resource = restlet.newClientResource("/sitemap_index_00001.xml");

        final Representation representation = resource.get();

        verify(feelhubSitemapModuleLink).get("/sitemap_index_00001.xml");
