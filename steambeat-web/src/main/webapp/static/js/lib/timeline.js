...
(function($) {
    ...
    this.buildBlocksFor = function(data, graph) {
        ...
        function buildFor(stat) {
            if(stat != null) {
                instance.orderOpinionsFor(stat);
                ...
                var emptystat = {
                    ...
                    opinions : [["good", 0],["bad", 0],["neutral", 0]]
                };
    ...
    this.orderOpinionsFor = function(stat) {
        var sortedOpinions= [];
        $.each(stat.opinions, function(index, element) {
            sortedOpinions.push([index, element]);
        });
        sortedOpinions.sort(function(a, b) {
            return b[1] - a[1];
        });
        stat.opinions = sortedOpinions;
    };

    this.normalizeData = function(stat) {
        var total = stat.opinions[0][1] + stat.opinions[1][1] + stat.opinions[2][1];
        for(var i = 0 ; i < stat.opinions.length; i++) {
            stat.opinions[i][1] = normalizeValue(stat.opinions[i][1]);
        }
    ...
    this.computeBlockFor =  function(stat, interval) {
        ...
        function setCoords() {
            ...
            for(var i = stat.opinions.length - 1 ; i >= 0 ; i--) {
                block.coords[stat.opinions[i][0]].top =  heightSum - stat.opinions[i][1];
                block.coords[stat.opinions[i][0]].bottom = heightSum;
                heightSum = heightSum - stat.opinions[i][1] - instance.spacer;
            }
        }
        
        function setSignLabel() {
            block.signLabel.text = (stat.opinions[0][1] != 0 ? instance.signs[stat.opinions[0][0]] : "");
            block.signLabel.y = parseFloat(block.coords[stat.opinions[0][0]].bottom / 2);
        }
